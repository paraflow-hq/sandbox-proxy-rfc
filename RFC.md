...
**说明：** 实验一至四使用运行时 pause/resume 路径验证。`setStartCmd` build 路径已通过 Suite 16（E2B Template SDK 构建真实模板，29/29 通过）单独验证。
...
| **应用层（真实 proxy-adapter）** | ✅ 已验证 | 204/204 | forwardViaWorker、DynamicCertManager、tunnelBypass、热切换、生产序列、崩溃恢复、压力测试、韧性 |
| **模板构建层（setStartCmd）** | ✅ 已验证 | 29/29 | Template SDK 构建、proxy + nft 从 sandbox 创建即生效、UID 豁免、snapshot/restore、多 sandbox 隔离、完整 RFC 生命周期 |
| **协议覆盖层（WebSocket）** | ✅ 已验证 | 25/25 | ws:// HTTP Upgrade 优雅处理、wss:// passthrough TCP 隧道全双工、wss:// MITM TLS 终止、并发韧性、UID 豁免 |
| **真实 passthrough + activate-mitm** | ✅ 已验证 | 24/24 | 真实 proxy-adapter.js 的 `--passthrough` + `/__activate-mitm`：passthrough 启动、HTTPS 真实证书、激活后 MITM 证书、snapshot 存活、幂等激活 |
| **生产精确路径（setStartCmd + 真实 passthrough）** | ✅ 已验证 | 24/24 | 真实 proxy-adapter.js --passthrough + setStartCmd 模板构建、passthrough 从 birth、activate-mitm、MITM 证书、二次 snapshot、多 sandbox 独立激活 |

验证代码：https://github.com/paraflow-hq/sandbox-proxy-rfc
...
| `setStartCmd` 集成路径 | ✅ 已通过 Suite 16 端到端验证（E2B Template SDK 构建真实模板，29/29 通过）。生产部署仍需在 `paraflow-hq/sandbox` template.py 上执行最终集成。 |
...
**参考实现：** `fixtures/passthrough-hotswitch.mjs`（本仓库，355 项测试验证）。真实 proxy-adapter.js 的 `--passthrough` + `/__activate-mitm` 已通过 Suite 18-19 端到端验证（含 setStartCmd 模板构建精确路径）。
